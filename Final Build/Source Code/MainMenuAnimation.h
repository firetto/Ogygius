...
This file is for the declaration of the main menu background animation
*/
...
private:
	...
	struct Square {
		...
		Square() {
			...
			// the scale is based on the window size compared to the default, just so it doesn't lag more when the window is huge
			float velocityScale = float(WINDOW_DIMENSIONS.x) / defaultWindowDimension.x;
			// random velocity
			velocity = getRandomFloat(velocityMin*velocityScale, velocityMax*velocityScale);
